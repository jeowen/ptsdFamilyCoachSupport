...
+(TUNinePatch*)ninePatchWithNinePatch:(TUNinePatch*)_src {
    TUFullNinePatch *src = (TUFullNinePatch*)_src;
    return [[[TUFullNinePatch alloc] initWithCenter:src.center
                                           contentRegion:src.contentRegion
                                    tileCenterVertically:src.tileCenterVertically
                                  tileCenterHorizontally:src.tileCenterHorizontally
                                         upperLeftCorner:src.upperLeftCorner
                                        upperRightCorner:src.upperRightCorner
                                         lowerLeftCorner:src.lowerLeftCorner
                                        lowerRightCorner:src.lowerRightCorner
                                                leftEdge:src.leftEdge
                                               rightEdge:src.rightEdge
                                               upperEdge:src.upperEdge
                                               lowerEdge:src.lowerEdge] autorelease];
}
...
+(id < TUNinePatch >)ninePatchNamed:(NSString *)filename {
	...
	if (filename) {
		...
		if (mainBundle) {
			NSString *filePath = [mainBundle pathForResource:[NSString stringWithFormat:@"%@@2x.9",filename] 
													  ofType:@"png"];
			if (filePath) {
...
NSRangeF NSMakeRangeF(float location, float length) {
    NSRangeF r;
    r.location = location;
    r.length =  length;
    return r;
...
+(CGRect)rectFromHorizontalRange:(NSRangeF)horizontalRange verticalRange:(NSRangeF)verticalRange {
