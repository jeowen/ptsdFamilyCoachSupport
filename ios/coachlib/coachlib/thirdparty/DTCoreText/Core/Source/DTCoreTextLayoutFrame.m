...
- (id)accessibilityElementAtIndex:(NSInteger)index {
	__block int count = 0;
	__block UIAccessibilityElement *r_elm = nil;
	[_attributedStringFragment enumerateAttribute:DTTextBlocksAttribute inRange:_stringRange options:0
									   usingBlock:^(NSArray *blockArray, NSRange range, BOOL *stop) {
										   for (DTTextBlock *oneBlock in blockArray) {
											   if (count++ == index) {
												   UIAccessibilityElement *elm = [[UIAccessibilityElement alloc] init];
												   ...
												   elm.accessibilityFrame = frame;
												   elm.accessibilityValue = [[[self attributedStringFragment] attributedSubstringFromRange:range] string];
												   r_elm = elm;
											   }
										   }
									   }];
