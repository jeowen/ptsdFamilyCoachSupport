...
- (void)longPressRecognized {
    ...
    indexPathOfRow = [self beforeMove:indexPathOfRow];
    ...
    indexPathOfMovingCell = [indexPathOfRow retain];
...
- (void)shuffleCellsOutOfWayOfDraggedCellIfNeeded {
    ...
    if (rowToMoveTo != nil ) {
        rowToMoveTo = [self.tableView.delegate tableView:self.tableView targetIndexPathForMoveFromRowAtIndexPath:indexPathOfMovingCell toProposedIndexPath:rowToMoveTo];
    }
...
- (UIView *)shadowViewWithFrame:(CGRect)frame andShadowPath:(CGPathRef)shadowPath {
	UIView *shadowView = [[[UIView alloc] initWithFrame:frame] autorelease];
