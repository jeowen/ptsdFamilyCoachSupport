...
public class DynamicListView extends ListView {
	...
	private final int SMOOTH_SCROLL_AMOUNT_AT_EDGE = 25;
    ...
    private ScrollView mProxyScroller;
	...
	private void makeCellMobile(int position) {
		if (position == -1) position = pointToPosition(mDownX, mDownY);
	...
	private BitmapDrawable getAndAddHoverView(View v) {
		...
		if (mHoverCell == null) {
            ...
            mHoverCellCurrentBounds = mHoverCell.copyBounds();
            mHoverCellCurrentBounds.bottom = mHoverCellCurrentBounds.top + h;
            mHoverCellCurrentBounds.right = mHoverCellCurrentBounds.left + w;
        }
    ...
    public boolean isDrawingBitmap() {
        return isDrawingBitmap;
    }
...
*/
	...
	private void handleCellSwitch() {
		...
		int deltaYTotal = (mHoverCellOriginalBounds.top+mHoverCellOriginalBounds.bottom)/2 + mTotalOffset + deltaY;

		View belowView = getViewForID(mBelowItemId);
		View mobileView = getViewForID(mMobileItemId);
		View aboveView = getViewForID(mAboveItemId);

		boolean isBelow = (belowView != null)/* && (getPositionForView(belowView) <= getHeaderViewsCount()) */&& (deltaYTotal > (belowView.getTop() + belowView.getBottom())/2);
		boolean isAbove = (aboveView != null) && (getPositionForView(aboveView) >= getHeaderViewsCount()) && (deltaYTotal < (aboveView.getTop() + aboveView.getBottom())/2);
	...
	public void touchEventsCancelled() {
		View mobileView = getViewForID(mMobileItemId);
        boolean callDragFinished = mCellIsMobile;
		if (mCellIsMobile) {
			mAboveItemId = INVALID_ID;
			mMobileItemId = INVALID_ID;
			mBelowItemId = INVALID_ID;
		...
		mIsMobileScrolling = false;
		mActivePointerId = INVALID_POINTER_ID;
        if (callDragFinished) onDragFinished();
    }
    ...
    public int getRealLastVisiblePosition() {
        ...
        Rect displayBounds = new Rect(0,0,dm.widthPixels,dm.heightPixels);
        while (pos >= 0) {
            View v = getChildAt(pos);
            if (v != null) {
                v.getLocationOnScreen(coords);
                if (displayBounds.intersects(coords[0], coords[1], coords[0] + v.getWidth(), coords[1] + v.getHeight())) {
                    break;
                }
            }
            pos--;
    ...
    public void setProxyScroller(ScrollView mProxyScroller) {
        this.mProxyScroller = mProxyScroller;
    }

    public void setProxyScroller() {
        ScrollView sv = null;
	...
	public boolean handleMobileCellScroll(Rect r) {
        ...
        if ((coords[1]+visibleHeight) > dm.heightPixels) visibleHeight = dm.heightPixels - coords[1];

		if (hoverViewTop <= 0 && offset > 0) {
//            if (getFirstVisiblePosition() >= 16) {
                if (mProxyScroller == null) {
        			smoothScrollBy(-mSmoothScrollAmountAtEdge, 0);
                } else {
                    mProxyScroller.smoothScrollBy(0,-mSmoothScrollAmountAtEdge);
                }
	    		return true;
//            }
		}

		if (hoverViewTop + hoverHeight >= visibleHeight) {
            if (getRealLastVisiblePosition() < (getCount()-16)) {
                if ((mProxyScroller == null) && (offset + extent) < range) {
        			smoothScrollBy(mSmoothScrollAmountAtEdge, 0);
                } else if (mProxyScroller != null) {
                    mProxyScroller.smoothScrollBy(0,mSmoothScrollAmountAtEdge);
                }
	    		return true;
            }
