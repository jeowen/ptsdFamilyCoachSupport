...
import gov.va.contentlib.questionnaire.Settings;
import gov.va.contentlib.questionnaire.android.QuestionnaireDatabase.PendingQuestionnaire;
...
public class QuestionnaireActivityBase extends Activity implements QuestionnaireListener {
	...
	@Override
	public void onShowScreen(QuestionnairePlayer player, ContentViewControllerBase newScreen) {
		if (currentScreen != null) {
			top.removeView(currentScreen);			
		}

		top.addView(newScreen.getView());
		
		if (currentScreen == null) {
			setContentView(veryTop);
		}
		currentScreen = newScreen.getView();
	}
